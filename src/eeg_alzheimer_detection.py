...
class Config:
    ...
    # EEG preprocessing parameters - based on actual data files
    CHANNELS_19 = [
    ...
    # Feature dimensions
    N_CHANNELS = 21  # Updated to match actual data
    N_FREQ_BANDS = 5
    N_PSD_FEATURES = N_CHANNELS * N_FREQ_BANDS  # 105
    N_PLI_FEATURES = (N_CHANNELS * (N_CHANNELS - 1)) // 2  # 210
    TOTAL_FEATURES = N_PSD_FEATURES + N_PLI_FEATURES  # 315
...
class EEGPreprocessor:
    ...
    def extract_features(self, raw):
        ...
        try:
            ...
            return combined_features  # Shape: (n_epochs, TOTAL_FEATURES)
        except Exception as e:
...
def load_dataset(data_dir, config):
        ...
        for condition in ['Eyes_closed', 'Eyes_open']:
            ...
            for patient in patient_dirs:
                ...
                try:
                    ...
                    if n_sequences > 0:  # Only process if we have at least one full sequence
    ...
    # Check shape consistency before converting to array
    print(f"\nChecking sequence shapes...")
    shapes = [seq.shape for seq in all_sequences[:5]]  # Check first 5
    ...
    for i, (seq, label, subject) in enumerate(zip(all_sequences, all_labels, all_subjects)):
        if seq.shape == expected_shape:
            valid_sequences.append(seq)
            valid_labels.append(label)
            valid_subjects.append(subject)
        else:
            print(f"Warning: Skipping sequence {i} with shape {seq.shape}, expected {expected_shape}")
    ...
    X = np.array(valid_sequences)  # Shape: (n_samples, sequence_length, features)
...
def main():
    ...
    # Save the best model from the best fold for cross-validation
    print(f"\n{'='*60}")
    print("SAVING BEST MODEL FOR CROSS-VALIDATION")
    print(f"{'='*60}")
    
    # Find the fold with best F1 score
    best_fold_idx = np.argmax([r['f1'] for r in results])
    best_fold_result = results[best_fold_idx]
    
    print(f"Best performing fold: {best_fold_result['fold']}")
    print(f"  F1-Score: {best_fold_result['f1']:.4f}")
    print(f"  Accuracy: {best_fold_result['accuracy']:.4f}")
    
    # Retrain on all data to get the final model
    print("\nRetraining on full dataset to create final model...")
    
    # Prepare full dataset
    X_train_full, y_train_full = torch.FloatTensor(X), torch.LongTensor(y)
    ...
    # Create new model using config object
    final_model = HybridCNNLSTM(config).to(device)
    
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(final_model.parameters(), lr=config.LEARNING_RATE)
    
    # Train for fewer epochs since this is final model
    num_final_epochs = min(20, config.NUM_EPOCHS)
    ...
    for epoch in range(num_final_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f"Epoch {epoch+1}/{num_final_epochs} - Loss: {avg_loss:.4f}")
    
    # Save the final model
    model_save_path = os.path.join(config.BASE_DIR, 'models', 'best_model.pth')
    os.makedirs(os.path.dirname(model_save_path), exist_ok=True)  # Ensure models directory exists
    torch.save({
        'model_state_dict': final_model.state_dict(),
        'best_val_acc': best_fold_result['accuracy'] * 100,
        'best_f1': best_fold_result['f1'],
        'config': {
            'N_CHANNELS': config.N_CHANNELS,
            'N_FREQ_BANDS': config.N_FREQ_BANDS,
            'N_PSD_FEATURES': config.N_PSD_FEATURES,
            'N_PLI_FEATURES': config.N_PLI_FEATURES,
    ...
    print(f"  This model can now be used for cross-validation testing.")
    print(f"\n  Run: python cross_validation_test.py")
    print(f"{'='*60}\n")
